...
## 17 September company-page continuity

The previous pass finished at `530e9611bd210b819beb395d550d121968013857`; Actions run `35135638743` passed and its final homepage/lab screenshots were inspected, including the corrected mobile camera framing. The user approved those visuals and asked to continue.

Baseline screenshots from run `35184362279` showed that the remaining company pages still used pale surfaces, oversized titles and a different reading hierarchy. The supporting pages now share the approved charcoal palette, copper actions, 1240 px measure and natural section heights. A scoped `company.css` controls their reading layouts without changing the homepage or lab grids. Obsolete paper-theme routing and its conflicting final background override were removed. Research sources, reference tables, roadmap exit conditions, principles, archive entries and contact links adapt to narrow screens; scientific text and numerical data are retained.

Added `tests/site-pages.browser.py` to the existing Actions workflow. It captures System, Research, Roadmap, Reference Architecture, About, Contact, Archive and 404 at 1440, 768, 390 and 320 px, checking loading, shared navigation, page errors and horizontal overflow. Run `35184896453` passed at `ec7386b4e1ac1f6ae5da766eae193a010346386c`, including the existing numerical, type, reference-calculation and WebGL/resource suites. Downloaded screenshots were visually inspected on desktop and mobile, including source rows, tables, figures, principles and lower-page transitions. That review identified three final presentation adjustments: constrain the 404 return button, break the contact email at its address boundary when needed, and retain a single-line footer wordmark on small screens. These adjustments require the following final CI captures; the PR description records the latest verified head.

Local generated-link and noindex/sitemap checks also pass. Cloud Browser local previews remain blocked; visual review uses CI artifacts. The branch remains a draft stacked on PR #9, with no merge or deployment.

## 17 September navigation and return visits

The company-page pass finished at `ccd7dbb7c1a7411b9f33d28c444cd0108623f525`. Run `35185374796` and its final screenshots confirmed the last contact, footer and 404 fixes; the user approved the result and asked to continue.

A focused navigation audit found two lifecycle gaps. The native mobile disclosure had no Escape/outside/focus-leave dismissal, and the homepage globe stopped on `pagehide` without restarting when restored from the browser's back/forward cache. The shared header now dismisses without stealing outside focus, returns focus to Menu on Escape, and moves focus between visible navigation controls across the mobile breakpoint. It remains a native disclosure and works without JavaScript. The globe resumes its animation loop and size observer on a cached `pageshow`, preserving the user's view and pause state.

Browser coverage now exercises skip links, keyboard menu operation, outside dismissal, focus when resizing, real link destinations and JavaScript-disabled navigation in company/lab layouts. The globe checks repeated cached-page lifecycle events and real browser Back from the lab. Cache eligibility varies by browser runner, so those are distinct checks. The PR description records the latest verified head and screenshot review; the approved visual composition and simulation remain unchanged.
