...
import { environment, type Design, type Result } from '../simulation/engine.js';
...
export default function Studies({design,rows,onRows,onClose,onSelect}:{design:Design;rows:StudyRow[];onRows:(r:StudyRow[])=>void;onClose:()=>void;onSelect:(r:Result)=>void}) {
  const lunar=environment(design).id==='moon';
  ...
  return <Modal title="One question. Several flights." onClose={()=>{stop();onClose();}} wide>
    <p className="modal-intro">{lunar?'Compare lunar recovery, material, release timing or payload at the supplied geometry.':'Recovery strategies carry different hardware masses. Power sweeps keep the hardware budget fixed, not automatically resized.'} Run the actual mission for every candidate. Compare what it delivers, not just how it looks.</p>
    <div className="study-controls"><label>Study variable<select aria-label="Study variable" value={kind} onChange={e=>setKind(e.target.value as StudyKind)} disabled={busy}>{Object.entries(STUDY_LABELS).filter(([id])=>!lunar||id!=='electrical').map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></label><button className="primary" onClick={runStudy} disabled={busy}>Run {studyDesigns(design,kind).length} variants →</button>{busy&&<button onClick={stop}>Cancel study</button>}</div>
    <p className="study-disclosure">{kind==='material'?'Dimensions stay fixed. Density, mass and the strength assumption change. Future carbon is hypothetical.':kind==='recovery'?lunar?'Coast has no propellant mass or thrust. Chemical recovery carries its stated finite budget. Lunar experiments use no Earth electrodynamic actuator.':'Coast removes fuel and E0-specific electrical mass. The base hub and terminal budget stays in every mode. Chemical carries fuel; E0 carries conductors and an explicit electrical-hardware allowance. Different mass is part of this system comparison, not an isolated thrust test.':kind==='electrical'?'Only the continuous bus cap changes. Conductor geometry, assumed current ceiling and hardware mass stay fixed; this is not automatic solar-array sizing. More power may not help a current- or voltage-limited circuit.':kind==='release'?`Only ${lunar?'Moon':'Earth'}-relative release phase changes. Payload, cable geometry, material and recovery stay fixed.`:'Only payload mass changes. The structure is not resized. Loads that exceed the chosen allowable stop the run.'}</p>
