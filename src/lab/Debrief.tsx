...
export default function Debrief({result:r,baseline,challenge,onClose,onEdit,onJump,onPin,onCompare,onRestore,canStudy}:{
  ...
  const download=()=>{
    const report={format:'tether-lab-flight-report',version:2,model:r.model,design:r.design,numerics:{stepSeconds:r.maxStep,cells:r.cells},outcome:r.outcome,reason:r.reason,deliveries:r.deliveries,approaches:r.approaches,rendezvous:r.rendezvous,dryMassKg:r.dryMass,propellantUsedKg:r.fuelUsed,minClearanceM:r.minClearance,minAxialMargin:r.minMargin,events:r.events,challenge:challenge?{id:challenge.id,passed,gates}:null,limitations:'Planar rigid-body educational model. Constructed, independently propagated ideal approaches with numerical capture checks; no atmosphere, elasticity, guidance, debris, electrodynamic or return-traffic recovery.'};
