...
export default function Debrief({result:r,baseline,challenge,units,onClose,onEdit,onJump,onPin,onCompare,onRestore,canStudy}:{
  canStudy:boolean;result:Result;baseline:Result|null;challenge:Challenge|null;units:StudioUnits;onClose:()=>void;
  onEdit:(tab:'structure'|'mission'|'recovery')=>void;onJump:(t:number)=>void;onPin:()=>void;onCompare:()=>void;onRestore:(r:Result)=>void;
}) {
  const env=environment(r.design),diagnosis=diagnose(r),gates=challenge?challengeGates(challenge,r):[], good=deliveries(r),passed=!!gates.length&&gates.every(g=>g.pass);
  const delta=baseline?designChanges(baseline.design,r.design):[];
  const distance=(meters:number,digits=0)=>`${f(shownValue(meters,'m',units),digits)} ${units.distance}`;
  const changed=(key:keyof Result['design'],value:string|number)=>{
    const source=key==='thrustN'?'N':key==='tipSpeedKms'?'km/s':key==='spanKm'||key==='altitudeKm'||key==='edLengthKm'?'km':null;
    return source&&typeof value==='number'?`${shownValue(value,source,units)} ${shownUnit(source,units)}`:String(value);
  };
  const download=()=>{
    const report={format:'tether-lab-flight-report',version:3,model:r.model,design:r.design,environment:{body:env.name,meanRadiusM:env.radius,muM3s2:env.mu,clearanceCutoffM:env.cutoff},numerics:{stepSeconds:r.maxStep,cells:r.cells},outcome:r.outcome,reason:r.reason,deliveries:r.deliveries,approaches:r.approaches,rendezvous:r.rendezvous,dryMassKg:r.dryMass,propellantUsedKg:r.fuelUsed,electrical:{model:r.design.recovery==='electrodynamic'?'E0-equatorial':null,busEnergyJ:r.electricalEnergyJ,mechanicalWorkJ:r.electricalWorkJ,heatAndDumpJ:r.electricalHeatJ,environmentExchangeJ:r.environmentEnergyJ,conductorMassKg:r.conductorMass,hardwareBudgetKg:r.electricalHardwareMass},minClearanceM:r.minClearance,minAxialMargin:r.minMargin,events:r.events,challenge:challenge?{id:challenge.id,passed,gates}:null,limitations:env.id==='moon'?'Spherical-Moon, planar rigid-body orbital experiment. Ideal independently propagated rendezvous; no surface pickup, terrain, mascons, reeling, flexible dynamics, Earth/Sun perturbations or Earth–Moon targeting. Chemical recovery or coast; no lunar electrodynamic actuator.':'Planar rigid-body educational model. Constructed, independently propagated ideal approaches with numerical capture checks; no atmosphere, elasticity, guidance, debris or return-traffic recovery. E0 electrodynamics assumes equatorial dipole, corotating plasma closure and constant electrical supply; no plasma, thermal, eclipse or power-system sizing prediction.'};
  ...
  return <Modal title="Flight debrief" onClose={onClose} wide>
    ...
    <div className="debrief-columns"><section><h4>The next experiment</h4><p>{diagnosis.action}</p><div className="debrief-buttons"><button className="primary" onClick={()=>onEdit(diagnosis.tab)}>Open {diagnosis.tab} controls →</button><button onClick={()=>onJump(diagnosis.time)}>Inspect this moment</button></div>
      ...
      <h4>Delivered trajectories · {env.name}</h4>{r.deliveries.length?<div className="delivered-orbits">{r.deliveries.map(d=><div key={d.number}><b>Payload {d.number}</b><span>{distance(d.perigee)} perigee / {d.apogee===null?'escape':`${distance(d.apogee)} apogee`}</span><small>{f(d.gain/1e6,2)} MJ/kg energy gain · {d.perigee>=env.cutoff&&d.gain>0?'delivery criterion met':'delivery criterion not met'}</small></div>)}</div>:<p>No payload reached release in this run.</p>}
      ...
      {challenge?<ul className="objective-list">{gates.map(g=><li key={g.label} className={g.pass?'gate-passed':'gate-open'}><span aria-label={g.pass?'Passed':'Not met'}>{g.pass?'✓':'○'}</span><div><b>{g.label}</b><small>{g.value}</small></div></li>)}</ul>:<p>Minimum tether clearance: <b>{distance(r.minClearance)}</b>. Minimum axial load margin: <b>{f(r.minMargin,2)}×</b>. These are model outputs, not safety certification.</p>}
      <p className="debrief-boundary">All results describe the calculated six-hour-or-shorter experiment. A released object is not counted as a successful delivery unless its energy increases and its perigee remains above the {distance(env.cutoff)} {env.name} cutoff.</p>
    </section></div>
    {baseline&&<section className="debrief-comparison"><div><h4>Against your pinned flight</h4><p>{delta.length?delta.map(k=>`${FIELD_NAMES[k]}: ${changed(k,baseline.design[k])} → ${changed(k,r.design[k])}`).join(' · '):'Identical design inputs. Repeatability comparison.'}</p></div><div className="comparison-table-wrap"><table><thead><tr><th>Full-run metric</th><th>Pinned</th><th>This run</th><th>Change</th></tr></thead><tbody>{[
      ['Successful deliveries',deliveries(baseline).length,good.length,''],['Dry facility',baseline.dryMass/1000,r.dryMass/1000,'t'],['Fuel used',baseline.fuelUsed/1000,r.fuelUsed/1000,'t'],['Bus energy',baseline.electricalEnergyJ/3.6e9,r.electricalEnergyJ/3.6e9,'MWh'],['Minimum clearance',shownValue(baseline.minClearance,'m',units),shownValue(r.minClearance,'m',units),units.distance],['Minimum load margin',baseline.minMargin,r.minMargin,'×']
