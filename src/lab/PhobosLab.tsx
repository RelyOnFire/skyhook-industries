...
export default function PhobosLab(){
  ...
  const [busy,setBusy]=useState(true),[error,setError]=useState(''),[notice,setNotice]=useState(''),[share,setShare]=useState('');
  ...
  const clock=useRef(0),worker=useRef<Worker|null>(null),request=useRef(0),file=useRef<HTMLInputElement>(null),runRef=useRef(result);
  ...
  const dirty=!!result&&JSON.stringify(result.design)!==JSON.stringify(design);
  function run(candidate:PhobosDesign,auto=false){
    try{
      const d=validatePhobos(candidate);worker.current?.terminate();setBusy(true);setPlaying(false);setError('');setNotice('');setShare('');
      const id=++request.current,w=new Worker(new URL('./phobos-worker.ts',import.meta.url),{type:'module'});worker.current=w;
      w.onerror=()=>{if(id!==request.current)return;setBusy(false);setError('The calculation worker could not start. Reload to try again.');w.terminate();};
      w.onmessage=e=>{if(id!==request.current)return;setBusy(false);w.terminate();worker.current=null;
  ...
  function adopt(d:PhobosDesign,auto=false,keepLink=false){if(!keepLink)history.replaceState(null,'','/lab/phobos/');setDesign(d);setBad([]);setFieldKey(k=>k+1);setInspected(d.release);run(d,auto);}
  useEffect(()=>{
    ...
    <div className="phobos-workspace">
      <aside className="lab-panel phobos-design" aria-label="Phobos design controls"><div className="panel-heading"><h2>Design bay</h2><span className="tag">{dirty?'UNRUN CHANGES':'RADIAL ARMS'}</span></div>
        <div className="phobos-design-content"><p className="phobos-help">Lengths begin at Phobos’ surface. Both arms turn with the moon around Mars.</p>
          ...
          <button className="primary phobos-run" disabled={busy||!!bad.length} onClick={()=>run(design,true)}>{busy?'Calculating…':'Run release →'}</button>
      ...
      <section className="lab-panel phobos-flight" aria-label="Phobos release flight" aria-busy={busy}>
        ...
        </>:<div className="phobos-loading">Calculating the Mars–Phobos flight…</div>}
      </section>
      <aside className="lab-panel phobos-results" aria-label="Release results" aria-busy={busy}>
        <div className="panel-heading"><h2>Flight brief</h2><span className={`tag${failed?' tag-amber':''}`}>{busy?'CALCULATING':dirty?'PREVIOUS RUN':failed?'LIMIT REACHED':'CALCULATED'}</span></div>
