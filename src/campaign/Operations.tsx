...
export function Outposts({world,busy,selected,onSelect,act,prepare}:{world:Campaign;busy:boolean;selected:SiteId;onSelect:(site:SiteId)=>void;act:Act;prepare:Prepare}) {
  return <section className="ops-outposts" id="outposts" aria-labelledby="outposts-heading">
    ...
    {SITES.map(id=>{
      ...
      const status=industryStatus(world,id),waiting=status.startsWith('Waiting')||status.includes('exhausted');
      ...
      return <article key={id} id={'outpost-'+id} className={'outpost'+(selected===id?' selected':'')+(locked?' locked':'')} aria-label={SITE[id].name+' outpost'}>
        ...
        {locked?<div className="outpost-locked"><p>Develop Mars operations to reach Mercury.</p><p><b>{n(world.marsOperations)} / 100</b> operations points</p><a href="#solar-heading">Mercury expedition ↗</a></div>:<>
          ...
          <p className={'outpost-industry'+(waiting?' needs-supply':'')}><span className="status-dot"/>{p.industry?status:'Industry not installed'}</p>
          {p.industry&&<p className="outpost-rate">{rate}{id==='mercury'&&world.solar.mirrorWorks&&world.solar.depositT===0?' · local deposit exhausted':''}</p>}
          <p className="outpost-rating">{p.level?p.readyDay>world.day?'Tether recovering · '+n(p.readyDay-world.day)+' days':'Tether ready · '+p.level*10+' t / flight':SITE[id].facility+' awaiting construction'}</p>
          ...
          {p.level<3&&<button className="outpost-build" disabled={busy||p.materialsT<cost} onClick={()=>act(w=>build(w,id))}>{p.level?'Upgrade':'Commission'} {machine} · {cost} t</button>}
