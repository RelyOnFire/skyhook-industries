import { BELT, beltProduction, ceresReserveT, buildPropellantWorks, ceresUnlockReason, propellantBuildReason, routeFor, unlockCeres, type Campaign } from './model.js';
...
export default function BeltChapter({world,busy,act,prepare}:{world:Campaign;busy:boolean;act:Act;prepare:Prepare}) {
  ...
  return <section className="campaign-belt" id="belt-operations" aria-labelledby="belt-heading">
    ...
    {!b.unlocked?<div className="belt-expedition"><p>Your swarm is growing. Build a second source of support propellant through the Phobos hub.</p><p><b>60 t material +20 t equipment at Phobos</b> funds the expedition. Ceres supplies travel separately.</p><button className="primary" disabled={busy||!!unlock} onClick={()=>act(unlockCeres)}>Open Ceres expedition</button><p className="tiny">{unlock||'Ready to depart. Keep supplies for the Ceres outpost and Phobos plant.'}</p></div>:<>
      ...
      <p className="belt-advice">Allow <b>{n(route.coastDays+route.handlingDays)} days each way</b>. Send 50 t material for the port and water works, plus 20 t equipment: 5 t for construction and 15 t to cover the first supply flight. {world.development.waterLevel===0?'Then send 5 t equipment every 200 days. Keep the Phobos hub supplied too.':'Expanded extraction needs up to '+n(p.waterCapacity*BELT.mineEquipmentPerT,2)+' t equipment/day. Increase recurring supplies and allow for the transfer lead time.'}</p>
      <div className="belt-deposit"><label htmlFor="ceres-deposit">Local water deposit <span>{n(b.depositT)} t remaining</span></label><progress id="ceres-deposit" value={b.extractedT} max={ceresReserveT(world)}/></div>
      <p className="belt-assumption">Water becomes fuel only at Phobos: up to {n(p.fuelCapacity)} t per day, with equipment and storage available. Freight consumes fuel too. <a href="/lab/campaign/method/#ceres">Scenario recipes & limits ↗</a></p>
    </>}
  </section>;
}
