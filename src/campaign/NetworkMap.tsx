...
export default function NetworkMap({world,selected,onSelect}:{world:Campaign|null;selected:SiteId;onSelect:(id:SiteId)=>void}) {
  return <div className="network-map">
    ...
    <svg viewBox="0 0 900 390" role="img" aria-label="Transport network: Earth and its Moon connected by the cislunar corridor; Earth connects to Phobos in the Mars system.">
      ...
      <path d="M220 200 L340 100 M220 200 L710 240" className="map-route"/>
      <text x="438" y="197" className="map-route-label">MARS TRANSFER CORRIDOR</text>
      ...
      {world?.flights.map(f=>{const t=Math.min(1,Math.max(0,(world.day-f.departed)/(f.arrival-f.departed))),a=POINTS[f.from],b=POINTS[f.to];return <g key={f.id}><circle cx={a[0]+(b[0]-a[0])*t} cy={a[1]+(b[1]-a[1])*t} r="5" fill="#efa477" stroke="#080b0d" strokeWidth="2"/><title>{`Flight ${f.id}: ${f.cargoT} t to ${SITE[f.to].name}`}</title></g>;})}
