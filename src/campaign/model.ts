...
export const CAMPAIGN_MODEL = 'network-0.2.0';
...
export interface Campaign {
  schema: 2; model: typeof CAMPAIGN_MODEL; id: string; name: string; revision: number;
  ...
  services: Service[]; nextService: number; marsOperations: number; lunarPhobosDeliveredT: number;
}
...
export function createCampaign(id: string, name: string): Campaign {
  ...
  return { schema: 2, model: CAMPAIGN_MODEL, id, name: name.trim().slice(0,48) || 'First light', revision: 0,
    day: 0, fuelT: 100, nextShipment: 1, nextSupplyDay: 0, lunarReturnedT: 0,
    ports: { earth: port(160,1), moon: port(0), phobos: port(0) }, flights: [],
    services: [], nextService: 1, marsOperations: 0, lunarPhobosDeliveredT: 0,
...
export function flightPlan(world: Campaign, from: SiteId, to: SiteId, cargoT: number, mode: Shipment['mode'], kind: CargoKind = 'materials') {
  ...
  let reason = '';
  if (!Object.hasOwn(CARGO,kind)) reason = 'Choose a cargo type.';
...
export function nextEventDay(world: Campaign) {
  const days = [...world.flights.map(f => f.arrival), ...SITES.map(s => world.ports[s].readyDay), world.nextSupplyDay,
    ...world.services.filter(s=>s.enabled).map(s=>s.nextDay)].filter(d => d > world.day + EPS && d <= LIMITS.days);
...
export function validateCampaign(value: unknown): Campaign {
  ...
  const raw = object(value), legacy=raw.schema===1 && raw.model==='network-0.1.0';
  if (!legacy && (raw.schema !== 2 || raw.model !== CAMPAIGN_MODEL)) throw Error('This save uses a different campaign version. Keep your backup; it has not been changed.');
  ...
  const flights: Shipment[] = raw.flights.map((v): Shipment => {
    ...
    if(legacy&&route.id==='lunar-mars')throw Error('Invalid first-chapter route.');
  ...
  return { schema:2, model:CAMPAIGN_MODEL, id:str(raw.id,80), name:str(raw.name,48), revision:num(raw.revision,0,1e9,true), day,
    ...
    services,nextService,marsOperations:legacy?0:num(raw.marsOperations,0,1e9),lunarPhobosDeliveredT:legacy?0:num(raw.lunarPhobosDeliveredT,0,1e9) };
}
export function exportCampaign(world: Campaign) { return JSON.stringify({ format:'skyhook-campaign', version:2, state:validateCampaign(world) },null,2); }
export function importCampaign(text: string, id: string): Campaign {
  ...
  if (envelope?.format !== 'skyhook-campaign' || ![1,2].includes(envelope?.version)) throw Error('Choose a Skyhook campaign backup. Flight Studio design files are separate.');
