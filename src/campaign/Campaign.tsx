    ...
    {world&&<>
      ...
      <div className="campaign-operations"><section className="campaign-dispatch" aria-labelledby="dispatch-heading"><p className="campaign-eyebrow">LOGISTICS / BOOK A FLIGHT</p><h2 id="dispatch-heading">Give the next outpost a beginning.</h2>
        <form onSubmit={e=>{e.preventDefault();act(w=>dispatch(w,from,to,Number(cargo),mode));}}>
          <div className="campaign-fields"><div><label htmlFor="campaign-origin">From</label><select id="campaign-origin" value={from} onChange={e=>chooseFrom(e.target.value as SiteId)}>{SITES.map(s=><option key={s} value={s}>{SITE[s].name}</option>)}</select></div><div><label htmlFor="campaign-destination">To</label><select id="campaign-destination" value={to} onChange={e=>setTo(e.target.value as SiteId)}>{(from==='earth'?(['moon','phobos'] as SiteId[]):(['earth'] as SiteId[])).map(s=><option key={s} value={s}>{SITE[s].name}</option>)}</select></div><div><label htmlFor="campaign-cargo">Construction cargo (t)</label><input id="campaign-cargo" type="number" min="1" max="30" step="1" required value={cargo} onChange={e=>setCargo(e.target.value)}/></div></div>
