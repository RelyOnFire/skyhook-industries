...
export default function Campaign() {
  ...
  const networkGoals=world?networkObjectives(world):[], solarGoals=world?solarObjectives(world):[], mercuryLocked=selected==='mercury'&&!world?.solar.unlocked, followed=world?.flights.find(f=>f.id===tracked);
  return <main ref={main} tabIndex={-1} id="lab-content" className="campaign">
    <header className="campaign-header"><div><p className="campaign-eyebrow">SKYHOOK / EXPEDITIONS</p><h1>{world?world.name:'A foothold. Then a network.'}</h1><p>{world?'Earth, the Moon, Phobos, and the first solar swarm.':'Build a transport network you can come back to.'}</p></div>
    ...
    {world&&<section className="campaign-ledger" aria-label="Network status"><div><span>SIMULATION CLOCK</span><strong data-testid="campaign-day">{day(world.day)}</strong><small>{playing?'Running · '+speed+' days per second':'Paused · no offline progress'}</small></div><div><span>CONSTRUCTION CARGO</span><strong>{number(SITES.reduce((sum,s)=>sum+world.ports[s].materialsT,0))} <em>t in depots</em></strong><small>{number(SITES.reduce((sum,s)=>sum+world.ports[s].equipmentT,0))} t equipment in depots</small><small>{number(world.flights.reduce((sum,f)=>sum+f.cargoT,0)+world.solar.deployments.reduce((sum,d)=>sum+d.massT,0))} t total cargo in transit</small></div><div><span>SUPPORT PROPELLANT</span><strong data-testid="campaign-fuel">{number(world.fuelT)} <em>t</em></strong><small>Network service allocation</small></div><div><span>COMMISSIONED TETHERS</span><strong>{SITES.filter(s=>world.ports[s].level).length} <em>/ {SITES.length}</em></strong><small>{world.flights.length+world.solar.deployments.length} active flights</small></div></section>}
    {world&&<nav className="campaign-chapters" aria-label="Campaign milestones"><a href="#progress-heading">First corridors <b>{goals.filter(g=>g.done).length}/4</b></a><a href="#network-goals">Working network <b>{networkGoals.filter(g=>g.done).length}/4</b></a><a href="#solar-heading">Solar swarm <b>{solarGoals.filter(g=>g.done).length}/4</b>{!world.solar.unlocked&&world.marsOperations>=100&&<span>EXPEDITION AVAILABLE</span>}</a></nav>}
    <div className="campaign-workspace"><NetworkMap world={world} selected={selected} onSelect={setSelected} tracked={tracked} playing={playing}/>
      <section id="campaign-facility" className="campaign-port" aria-label={`${SITE[selected].name} facility`}><div className="campaign-panel-heading"><span className="campaign-eyebrow">{SITE[selected].name.toUpperCase()} / FACILITY</span><span className="campaign-badge">{port?.level?`TIER ${port.level}`:'UNBUILT'}</span></div><h2>{SITE[selected].facility}</h2><p>{SITE[selected].description}</p><FacilityDrawing site={selected}/>
        {world&&port&&!mercuryLocked?<><dl className="campaign-specs"><div><dt>Construction cargo</dt><dd data-testid="port-materials">{number(port.materialsT)} t</dd></div><div><dt>Equipment</dt><dd data-testid="port-equipment">{number(port.equipmentT)} t</dd></div><div><dt>Service rating</dt><dd>{port.level?`${10*port.level} t / flight`:'Awaiting commissioning'}</dd></div><div><dt>Recovery</dt><dd>{port.readyDay>world.day?`Ready ${day(port.readyDay)}`:'Ready'}</dd></div></dl>
        ...
        </>:<p className="campaign-hint">{mercuryLocked&&world?<>Open the <a href="#solar-heading">Mercury expedition in Chapter 03</a> to reach this destination.</>:'Start a network to supply and commission this facility.'}</p>}
        {world&&port&&!mercuryLocked&&<div className="campaign-industry"><p className="campaign-eyebrow">LOCAL INDUSTRY</p><h3>{INDUSTRY[selected].name}</h3><p>{INDUSTRY[selected].detail}</p><p className="industry-status">{industryStatus(world,selected)}</p>
        ...
        <a className="campaign-text-link" href={selected==='earth'?'/lab/':selected==='moon'?'/lab/campaign/method/#lunavator':selected==='mercury'?'/lab/campaign/method/#mercury':'/lab/campaign/method/#phobos'}>{selected==='earth'?'Explore Earth tether physics':'Explore this architecture'} <span aria-hidden="true">↗</span></a>
      </section>
    </div>
    {world&&<>
      <section className="campaign-clock" aria-label="Advance simulation"><div><strong>{playing?'Network running':'Your next move'}</strong><span>{event!==null?'Next production, arrival, service, recovery, or supply: '+day(event):'Book a flight or request supplies.'}</span><span>Production runs with simulation time. Hidden tabs pause.</span></div><div className="campaign-time-controls">
      ...
      <div className="campaign-operations"><section className="campaign-dispatch" aria-labelledby="dispatch-heading"><p className="campaign-eyebrow">LOGISTICS / BOOK A FLIGHT</p><h2 id="dispatch-heading">Give the next outpost a beginning.</h2>
        ...
        {goals.every(g=>g.done)&&<div className="campaign-achievement" role="status"><b>The first network is established.</b><p>Keep developing your corridors, try a different network, or download this milestone. Continue through the working network toward the Mercury expedition and your first solar swarm.</p></div>}
      ...
      <SolarChapter world={world} busy={busy} act={act} onSelect={setSelected}/>
    ...
    <footer className="campaign-footer"><span>EXPEDITIONS / FIRST LIGHT</span><a href="/lab/campaign/method/">Assumptions & research</a><a href="/lab/">Open Flight Studio</a></footer>
  </main>;
}
